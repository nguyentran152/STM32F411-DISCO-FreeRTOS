...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	sprintf(str,"Demo of Binary semaphore usage between 2 Tasks \r\n");
	printmsg(str);
    ...
    if( Sema_key != NULL)
    {
        ...
        /* Make sema available for first time */
        xSemaphoreGive(Sema_key);
        /* Start the scheduler so the created tasks start executing. */
        vTaskStartScheduler();
    }else{
		sprintf(str,"Sema create failed.. \r\n");
		printmsg(str);
    }
...
static void vTask1( void *pvParameters )
{
	...
	while(1)
	{
		xSemaphoreTake( Sema_key, 0 );

		sprintf(str,"%s", pcTaskName);
		printmsg(str);
